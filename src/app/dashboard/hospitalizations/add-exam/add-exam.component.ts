...
export class AddExamComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.medication.frequency = '1'
    this.medication.itemquantity = 1
    this.medication.item_unity = 1
    this.medication.toPetQuantity = 1
    this.medication.start = '2021-06-21'
    this.medication.end = '2021-06-21'
  }
  ...
  createMed() {
    ...
    this.blockUI.start()
    this.medService.createMedication(this.medication).subscribe({
      next: res => {
        this.toastrService.success('Medicação cadastrada com sucesso', '', toastrConfig)
        this.closeModal(res)
      }
  ...
  getProds(search: any) {
    this.blockUI.start()
    this.prodService.getProds({ search: search, tipo: 'exames' }).subscribe({
      next: res => {
  ...
  doFilterMeasure() {
    ...
    if (typeof this.filterMed === 'object') {
      ...
      this.medication.total_value = this.filterMed.value
